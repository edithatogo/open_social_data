...
- [x] Task: Conductor - Review CLI command phase against current CLI integration tests
...
- [x] Task: Conductor - Review CLI UX phase against current CLI integration tests

## Swarm Notes - 2026-06-15
- Added `open-social-data-cli` entry point with `list`, `status`, and `fetch` subcommands.
- CLI validation now passes under the Windows GNU temp-target validation path.
